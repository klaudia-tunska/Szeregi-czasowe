...
legend("topleft", legend = c(2, 4, 6), col = c('red', 'blue', 'green'),pch=20, 
       title = 'Parametry średniej', bty="n")
...
legend("topleft", legend = c(0.01, 0.03, 0.05), col = c('red', 'blue', 'green'),pch=20, 
       title = 'Parametry regresji', bty="n")

# porownanie obu metod
plot(time_series, main = 'Wygładzanie szeregu', ylab = 'Ilość wyszukań', xlab = 'Czas')
lines(ts_filter, col = 'red')
lines(ts_lowess, col = 'blue')
legend("topleft", legend = c('średnia ruchoma', 'regresja nielokalna'), col = c('red', 'blue'),pch=20, title = 'Metoda wygładzania', bty="n")

```

Na powyższych wykresach widzimy dwie metody, które można używać do wygładzania szeregu, metodę średniej ruchomej oraz metodę regresji nielokalnej. Obie metody wydają się dobrze wygładzać nasz model, otrzymują one zbliżone, ale nie identyczne wyniki. Na podstawie samych wykresów nie da się stwierdzić, która metoda jest lepsza, dlatego na razie wstrzymujemy się od ich oceny.
...
legend("topleft", 
       ...
       col = c('blue', 'red', 'green', 'magenta'),pch=20, title = 'Metoda estymacji trendu', bty="n")
...
plot(ts-sez, xlab="Rok", ylab="Wartosc", main = "Szereg czasowy bez trendu i sezonowości")
lines(ts2-trend2, col='green')
legend("topleft", 
       legend = c('1. Trend, 2. Sezonowość', '1. Sezonowość, 2. Trend'), 
       col = c('black', 'green'),pch=20, title = 'Kolejność usuwania', bty="n")
...
Dokonamy teraz finalnej dekompozycji naszego szeregu, przy użyciu kilku różnych metod.
...
plot(ts1, main = 'Szereg czasowy po usunięciu trendu i sezonowości', ylab = 'Ilość wyszukań', xlab = 'Czas', ylim = c(-30,50))
lines(ts2, col = 'red')
lines(ts3, col='blue')
legend("topleft", 
       legend = c('poly,tslm', 'diff', 'decompose'), 
       col = c('black', 'red','blue'),pch=20, title = 'Metoda estymacji', bty="n")

plot(ts1, main = 'Szereg czasowy po usunięciu trendu i sezonowości', ylab = 'Ilość wyszukań', xlab = 'Czas',  ylim = c(-30,50))
lines(ts4, col = 'red')
lines(ts5, col='blue')
legend("topleft", 
       legend = c('oryginalne dane', 'średnia ruchoma', 'regresja nielokalna'), 
       col = c('black', 'red','blue'),pch=20, title = 'Metoda wygładzania', bty="n")
```

Na pierwszym z powyższych wykresów widzimy na czarno szereg, z którego usunęliśmy trend metodą wielomianową i szereg za pomocą funkcji tslm, na czerwono korzystaliśmy  z funkcji diff do usunięcia trendu i sezonowości, a na niebiesko korzystaliśmy z funkcji decompose. Wyniki dla diff znacznie odbiegają od wyników dla pozostałych metod, mają one duże piki. Wyniki dla pierwszej i trzeciej metody miejscami różnią się, ale ciężko powiedzieć, która jest lepsza, tylko na podstawie wykresu. Czynnikiem decydujący przy wyborze metody jest to, czy istnieje oczywisty sposób na przywrócenie trendu i sezonowości po ich odjęciu. W przypadku diff i decompose nie ma takiego sposobu, dlatego wybieramy pierwszą metodę.

Na drugim  wykresie widzimy wygładzenie naszego modelu dwoma metodami. Tak, jak wcześniej na razie ciężko stwierdzić, która metoda jest lepsza, dlatego rozważamy je obie w dalszej części raportu.
...
```{r modele1,echo = F,fig.align='center', out.width='80%', warning=FALSE}
...
### Dobór modelu dla szeregu wygładzonego metodą regresji nielokalnej

```{r modele,echo = F, fig.align='center', out.width='80%', warning=FALSE}
...
```
```{r rarimawykres2, echo = F, fig.align='center', out.width='80%'}
knitr::include_graphics('sarima2.png')
```
...
W końcowej części raportu zajmiemy się predykcją kolejnych wartości analizowanego szeregu czasowego. Naszym celem będzie wyznaczenie prognoz dla zainteresowania rowerami na kolejne 2 lata. W tym celu omówimy wybrane metody predykcji i wybierzemy najbardziej optymalną z nich. Prognozy wykonamy dla oryginalnego szeregu czasowego oraz dla szeregu wygładzonego metodą regresji nielokalnej. W ostatnim kroku porównamy wyniki dla obu podejść oraz przedstawimy ich wady i zalety. 
...
```{r forecast_metodyestymacji,echo = F,fig.align='center', out.width='80%'}
#model1 3 0 4
...
```{r forecasts_mse,echo = F,fig.align='center', out.width='80%'}
#bledy
...
par(mfrow=c(1,2))
plot(indexes, mse$sarima, ylim = c(0,1000), type = 'l', col = 'red', main ='MSE predykcji', xlim = c(0, 24), ylab = 'Wartość MSE', xlab = 'Czas')
...
legend("top", legend=c("sarima.for()", "forecast()", "ses()", "hw()", "holt()"),  fill = c("red", "blue", "green", "magenta", "yellow"), bty="n")

plot(indexes, mse$sarima, ylim = c(0,250), type = 'l',col = 'red', main ='MSE predykcji', xlim = c(0, 24), ylab = 'Wartość MSE', xlab = 'Czas')
lines(indexes, mse$forecast, col='blue')
lines(indexes, mse$hw, col = 'magenta')
legend("top", legend=c("sarima.for()", "forecast()", "hw()"),  fill = c("red", "blue", "magenta"), bty="n")

```

MSE dla metod *holt()* i *ses()* przyjmuje bardzo duże wartości. Błędy dla pozostałych modeli są znacznie mniejsze. MSE dla prognozowania metodą *sarima.for()* zdaje się być najbardziej optymalne.    
...
Dla regresji nielokalnej postępujemy analogicznie jak w przypadku opisanym powyżej.
...
plot(indexes, mse$sarima, ylim = c(0,250), type = 'l',col = 'red', main ='MSE predykcji', xlim = c(0, 24), ylab = 'Wartość MSE', xlab = 'Czas')
lines(indexes, mse$hw, col = 'magenta')
legend("top", legend=c("sarima.for()","hw()"),  fill = c("red", "magenta"), bty="n")
...
Funkcja *sarima.for()* minimalizuje MSE także dla szeregu wygładzonego metodą regresji nielokalnej. Zauważmy, że w tym przypaku MSE wynosi $354.1691$ i jest znacznie mniejsze niż dla szeregu bez wygładzania (MSE = $672.4980$). Metody *ses()* oraz *holt()* także wypadły lepiej. Funkcja $hw()$ w obu przypadkach zwróciła niemal identyczne wartości. Oznacza to, że dla wygładzonego szeregu łatwiej jest dokonać predykcji. 
...
Porównajmy teraz wyniki predykcji otrzymane dwiema metodami. Widzimy, że wykresy obu szeregów czasowych są do siebie podobne, co może świadczyć o dobrych wartościach predykatorów. Różnice zauważalne są jedynie przy wartościach odstających, co zapewne spowodowane jest wygładzaniem szeregu. 
...
Z wykresu można wnioskować, że sezonowość utrzyma się na tym samym poziomie. Nastąpi jednak lekkie wygładzenie trendu. Ogólna popularność rowerów nie spadnie ale też nie nastąpi znaczący wzrost. 

# Wnioski

## Wnioski statystyczne 

1. Przed właściwym modelowaniem szeregu czasowego należy nabrać intuicji na temat danych.
Za pomocą prostych wykresów dobrze jest sprawdzić istnienie trendu, sezonowości oraz kompletność danych. 
2. Przed dobraniem modelu można rozważyć także wygładzenie szeregu np. metodą średniej ruchomej, czy regresji wielorakiej. 
3. Istnieje wiele sposobów modelowania szeregu. Zadaniem statystyka jest dobrać ten, który najlepiej oddaje zachowanie danych. Ważne jest aby dobrany model był optymalny i łatwy do analizy. Dokładność doboru można badać przykładowo dzięki wskaźnikom: R kwadrat, AIC i BIC.  
4. Przed dokonaniem predykcji dobrze jest sprawdzić, która z metod prognozowania jest najlepsza dla analizowanego szeregu. Przykładowo można to zrobić dzieląc dane na zbiór treningowy i zbiór testowy. Przez prognozowanie na podstawie zbioru treningowego i porównanie wyników ze zbiorem testowym dowiadujemy się, jakie błędy popełniamy. 
